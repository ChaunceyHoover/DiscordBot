		...
		dbHelper.hasBeenVoted(msg)
			.then((voted, yay, nay, datetime) => {
				if (voted) {
					...
					msg.reply(`This message has already been voted on at ${datetime.toLocaleDateString() 
						+ ' ' + datetime.toLocaleTimeString()}. The result from voting was ${outcome}.`);
				} else {
					msg.react('✅')
						.then(() => msg.react('❌'))
						.then(() => {
							let name = msg.member.nickname || user.username;
							msg.channel.send(`Voting process started. Decide ${name}'s fate now.`)
								.then(sentMsg => {
									...
									{ max: msg.guild.memberCount * 2, time: 10000 }).then(collected => {
											...
											.then(() => {
												if (yay - nay > 0) {
													dbHelper.getPunishRole(msg.guild.id)
														.then(roleId => {
															sentMsg.edit(`Voting done! ${name} is guilty! 2 minutes jail!`);
													...
													sentMsg.edit(`Voting done! ${name} is not guilty!`);
												}
											})	
											.catch(err => {
												console.error(err);

...
client.on('messageReactionAdd', (reaction, user) => {
	...
	dbHelper.getIgnoreChannels(msg.guild.id)
		.then(channels => {
			if (channels.length == 0 || !channels.includes(msg.channel.id)) {
				if (emoji.name == '🔞' && msg.author.id != client.user.id) {
					dbHelper.hasBeenVoted(msg)
						.then((result) => {
							if (result.Voted) {
								...
								msg.reply(`This message has already been voted on at ${result.Time.toLocaleDateString() 
									+ ' ' + result.Time.toLocaleTimeString()} EST.\n\nThe result from voting was **${outcome}**.`);
							} else {
								msg.react('✅')
									.then(() => msg.react('❌'))
									.then(() => {

					...
					dbHelper.hasBeenVoted(msg)
						.then((result) => {
							if (result.Voted) {
								...
								let ESTtime = new Date(result.Time.setHours(result.Time.getHours() - 5));
								...
								msg.react('✅')
									.then(() => msg.react('❌'))
									.then(() => {
										client.user.setPresence({ activity: { name: 'HORNY DETECTED', type: 'WATCHING' }, status: 'online' })
											.catch(console.error);
										...
										msg.channel.send(`Voting process started. Decide ${name}'s fate now.`)
											.then(sentMsg => {
												...
												{ max: msg.guild.memberCount * 2, time: 45 * 1000 }).then(collected => {
													...
													const tampered = yay < 0 || nay < 0;
													
													dbHelper.registerMessageVoted(msg, tampered ? 0 : yay, tampered ? 0 : nay)
														.then(() => {
															dbHelper.getPunishRole(msg.guild.id)
																.then(roleId => {
																	if (tampered) {
																		sentMsg.edit("There's tomfoolery going on... Nobody gets punished since the votes were tampered 😡");
																	} else if (yay - nay > 0) {
														...
														.catch(err => {
															console.error(err);
															msg.channel.send('Unable to register message as voted. Democracy has been lost. You are forever doomed. Abandon all hope now.\n'
																+ `\`\`\`\n${err}\`\`\``);
															return;
														});
												});
